...
[![ci](https://github.com/ismailubts/kvgate/actions/workflows/ci.yml/badge.svg)](https://github.com/ismailubts/kvgate/actions/workflows/ci.yml)

**kvgate** is a single-binary OpenAI-compatible LLM gateway written in C++20.
It fronts self-hosted backends — vLLM, llama.cpp-server, and Ollama — behind one
endpoint, with SSE streaming, admission control, circuit-breaker failover, and
prefix-affinity routing. No Kubernetes, no control plane.

Self-hosted inference almost never means one engine. You usually mix a vLLM box,
a llama.cpp-server, and an Ollama instance, while apps still expect a single
OpenAI-compatible URL, token streaming that keeps inter-token timing, failover
when a backend dies, and sticky routing so multi-turn chats reuse the backend
that already cached their prefix. kvgate is that glue layer: one binary, with
per-route TTFT/ITL instrumentation on Prometheus.

The differentiator is the package deal — compact form factor, three engines in
one place, and latency histograms that are useful for measuring gateway
overhead. C++20 is used so the streaming hot path (re-emit each SSE frame with
a flush) stays under direct control; that is a deliberate choice, not a claim
that the job requires C++.
...
> preliminary WSL2 overhead figures only, and labeled as such.
...
**2. Build kvgate** (needs CMake ≥ 3.28, GCC 13 or Clang 18, Boost ≥ 1.83; see
[Build from source](#build-from-source) for the exact apt line):

```bash
git clone https://github.com/ismailubts/kvgate.git
cd kvgate
...
```bash
./build/kvgate --config config/ollama-quickstart.toml
# kvgate 0.1.0 ready — config/ollama-quickstart.toml (1 backend(s), policy=prefix_affinity)
```
...
[`config/kvgate.example.toml`](config/kvgate.example.toml).
...
The model map is authoritative: kvgate routes a request to a backend whose
`models` list contains the requested model id, and never rewrites model names
heuristically.

## Routing

kvgate ships two routing policies, selected by `routing.policy`. Round-robin is
...
3. **Load guard.** If the HRW winner is already at or above
   ...
   counted (`kvgate:affinity_spills_total`).

The bet is that pinning same-prefix traffic to one backend raises that backend's
engine-side prefix-cache hit rate, which lowers TTFT — without kvgate needing to
...
names are **colon-prefixed** (`kvgate:ttft_seconds`, …) on purpose: vLLM and
SGLang ship their serving metrics with a `vllm:` / `sglang:` colon prefix, and
matching that convention lets kvgate drop into the same GenAI dashboards.
...
| metric | meaning |
|--------|---------|
| `kvgate:ttft_seconds` | time to first token |
| `kvgate:inter_token_latency_seconds` | inter-token (time-per-output-token) latency |
| `kvgate:e2e_request_latency_seconds` | end-to-end request latency |
| `kvgate:request_queue_time_seconds` | time spent in the admission queue |
| `kvgate:gateway_overhead_seconds` | gateway self-overhead: admission to first upstream byte forwarded |

Gauges: `kvgate:requests_running`, `kvgate:requests_waiting`.
Counters: `kvgate:requests_total{code}`, `kvgate:affinity_spills_total`,
`kvgate:backend_failures_total`.

Histogram and counter series carry `{route, backend, model}` labels.
`kvgate:gateway_overhead_seconds` is the differentiator metric: it isolates the
latency kvgate itself adds, separate from whatever the backend takes.

## Benchmarks

There are two benchmark questions, and only one of them has numbers yet.

**Gateway overhead — preliminary (WSL2).** How much latency does kvgate add on
...
`kvgate:gateway_overhead_seconds` (admission → first upstream byte, over 4004
...
- **Ollama is fronted through its `/v1/` layer only.** kvgate talks to Ollama's
...
The binary is `build/kvgate`. Run it with `--config <path.toml>`; `--version` and
