# kvgate preliminary results — WSL2 (NOT publishable)
...
- Gateway: `kvgate` (RelWithDebInfo), `policy = round_robin`, one mock backend at
  **zero inter-token delay** (so the measured latency is dominated by the
  gateway + loopback, isolating kvgate's own overhead).
- Loadgen: Poisson arrivals, concurrency 128, overlap 0.0, max_tokens 32,
  warm-up 3 s excluded, duration 12 s per cell.

## Gateway-overhead experiment (loadgen → kvgate → mock  vs  loadgen → mock direct)
...
### Self-measured cross-check (`kvgate:gateway_overhead_seconds`)
...
## CPU flamegraph
`flamegraph_gateway_preliminary_wsl2.svg`
...
logic. No egregious hotspot in kvgate code → no optimization pass taken.
...
- `flamegraph_gateway_preliminary_wsl2.svg` — the CPU flamegraph.
