#!/usr/bin/env bash
# kvgate gateway-overhead experiment (preliminary, WSL2).
#
# Measures kvgate's own added latency by comparing, at matched load:
#   A) loadgen -> kvgate -> mock backend (zero inter-token delay)
#   B) loadgen -> mock backend directly
# The per-request TTFT/E2E delta between A and B is the gateway's self-overhead.
# kvgate also reports kvgate:gateway_overhead_seconds (admission -> first upstream
...
# launches kvgate against it, sweeps a few Poisson rates through both paths, writes
...
if [[ ! -x "$KVGATE_BIN" ]]; then
    echo "error: kvgate binary not found at $KVGATE_BIN (build first)" >&2
    exit 1
fi
...
echo "=== kvgate gateway-overhead experiment — preliminary (WSL2) ===" | tee "$SUMMARY"
...
# 2) kvgate config pointing at the mock (round_robin so routing cost is trivial).
GW_CFG="$(mktemp)"
cat > "$GW_CFG" <<EOF
[server]
listen = "127.0.0.1"
...
"$KVGATE_BIN" --config "$GW_CFG" > "$OUT_DIR/KVGATE_${STAMP}.log" 2>&1 &
GW_PID=$!
sleep 2
...
echo "=== /metrics scrape (kvgate:gateway_overhead_seconds cross-check) ===" | tee -a "$SUMMARY"
curl -s "http://127.0.0.1:$METRICS_PORT/metrics" > "$OUT_DIR/metrics_${STAMP}.prom" || true
grep -E '^kvgate:(gateway_overhead_seconds|ttft_seconds|e2e_request_latency_seconds)_(sum|count)' \
