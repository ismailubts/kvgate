...
namespace config = kvgate::config;
namespace server = kvgate::server;
  ...
  public:
    ...
    void mark_all_healthy() {
        for (std::size_t i = 0; i < n_; ++i) {
            gateway_->set_health_for_test(i, kvgate::health::HealthState::Healthy);
        }
    }
    void set_health(std::size_t i, kvgate::health::HealthState s) {
...
TEST_CASE("m3: circuit breaker opens after repeated failures and stops routing to the backend",
    ...
    CHECK(h.gateway().breaker_of(0) == kvgate::health::BreakerState::Open);
...
TEST_CASE("m3: NOT_READY backend is not eligible but a healthy one serves", "[m3][health]") {
    ...
    h.set_health(0, kvgate::health::HealthState::NotReady);
    h.set_health(1, kvgate::health::HealthState::Healthy);
    for (int i = 0; i < 6; ++i) {
        auto r = do_chat(h.port(), kChat);
...
TEST_CASE("m3: no healthy backend yields 503", "[m3][health]") {
    MockBackend a;
    Harness h({a.port()});
    h.set_health(0, kvgate::health::HealthState::Dead);
    auto r = do_chat(h.port(), kChat);
    CHECK(r.status == 503);
...
TEST_CASE("m3: live health monitor probes each backend type correctly", "[m3][health]") {
    ...
    MockBackend mb;
    asio::io_context ioc;
    kvgate::upstream::Client client(
        ioc, kvgate::upstream::Target::parse("http://127.0.0.1:" + std::to_string(mb.port())));

    auto probe = [&](config::BackendType type) -> kvgate::health::HealthState {
        kvgate::health::HealthMonitor mon(ioc, type, client, 2000ms);
    ...
    SECTION("vLLM /health 200 -> HEALTHY") {
        ...
        CHECK(probe(config::BackendType::Vllm) == kvgate::health::HealthState::Healthy);
    }
    SECTION("llama.cpp /health 503 -> NOT_READY") {
        mb.set_mode(MockBackend::Mode::Health503);
        CHECK(probe(config::BackendType::Llamacpp) == kvgate::health::HealthState::NotReady);
    }
    SECTION("llama.cpp /health 500 -> DEAD") {
        mb.set_mode(MockBackend::Mode::HealthDead);
        CHECK(probe(config::BackendType::Llamacpp) == kvgate::health::HealthState::Dead);
    }
    SECTION("Ollama GET / banner + /api/tags 200 -> HEALTHY") {
        mb.set_mode(MockBackend::Mode::Ok);
        CHECK(probe(config::BackendType::Ollama) == kvgate::health::HealthState::Healthy);
    }
    SECTION("Ollama live but /api/tags 503 -> NOT_READY") {
        mb.set_mode(MockBackend::Mode::Health503);
        CHECK(probe(config::BackendType::Ollama) == kvgate::health::HealthState::NotReady);
    }
}
