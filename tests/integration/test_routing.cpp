...
TEST_CASE("m4: counters and gauges move under load", "[m4][metrics]") {
    ...
    auto pos = m.find("kvgate:requests_total{code=\"200\"}");
    REQUIRE(pos != std::string::npos);
    int total = std::stoi(m.substr(m.find(' ', pos) + 1));
    CHECK(total >= 20);

    // TTFT and ITL histograms recorded observations.
    CHECK(m.find("kvgate:ttft_seconds_count{") != std::string::npos);
    CHECK(m.find("kvgate:inter_token_latency_seconds_count{") != std::string::npos);
    CHECK(m.find("kvgate:e2e_request_latency_seconds_count{") != std::string::npos);

    // Gauges are present (settle to 0 in-flight after the load drains).
    CHECK(m.find("kvgate:requests_running ") != std::string::npos);
    CHECK(m.find("kvgate:requests_waiting ") != std::string::npos);
}
...
TEST_CASE("m4: round_robin remains selectable and spreads", "[m4][rr]") {
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        gw.set_health_for_test(i, kvgate::health::HealthState::Healthy);
    }
