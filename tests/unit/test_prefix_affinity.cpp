...
using kvgate::routing::Candidate;
using kvgate::routing::PrefixAffinity;
using kvgate::routing::RouteRequest;
...
std::string canon(const std::string& model,
    ...
    return kvgate::routing::canonical_affinity_string(model, msgs, bytes);
}
} // namespace
...
TEST_CASE("affinity: same prefix -> same key hash; different prefix -> different",
          "[affinity][key]") {
    auto k1 = kvgate::routing::affinity_key_hash(canon("m", {{"user", "shared prefix abc"}}));
    auto k2 = kvgate::routing::affinity_key_hash(canon("m", {{"user", "shared prefix abc"}}));
    auto k3 = kvgate::routing::affinity_key_hash(canon("m", {{"user", "totally different"}}));
    CHECK(k1 == k2);
    CHECK(k1 != k3);
}

TEST_CASE("affinity: model is part of the key (same content, different model)", "[affinity][key]") {
    auto k1 = kvgate::routing::affinity_key_hash(canon("model-a", {{"user", "x"}}));
    auto k2 = kvgate::routing::affinity_key_hash(canon("model-b", {{"user", "x"}}));
    CHECK(k1 != k2);
}
...
TEST_CASE("affinity: under-loaded winner is chosen with zero spills", "[affinity][spill]") {
    ...
    auto rank =
        PrefixAffinity::hrw_rank(kvgate::routing::affinity_key_hash(req.affinity_key), cands);
...
TEST_CASE("affinity: overloaded winner spills to the next HRW candidate", "[affinity][spill]") {
    ...
    std::uint64_t key = kvgate::routing::affinity_key_hash(req.affinity_key);
