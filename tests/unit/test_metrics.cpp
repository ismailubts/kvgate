...
using kvgate::metrics::Labels;
using kvgate::metrics::Registry;
...
TEST_CASE("metrics: exposition contains all locked metric names and TYPEs", "[metrics]") {
    ...
    CHECK(out.find("kvgate:ttft_seconds") != std::string::npos);
    CHECK(out.find("kvgate:inter_token_latency_seconds") != std::string::npos);
    CHECK(out.find("kvgate:e2e_request_latency_seconds") != std::string::npos);
    CHECK(out.find("kvgate:request_queue_time_seconds") != std::string::npos);
    CHECK(out.find("kvgate:gateway_overhead_seconds") != std::string::npos);
    CHECK(out.find("kvgate:requests_running") != std::string::npos);
    CHECK(out.find("kvgate:requests_waiting") != std::string::npos);
    CHECK(out.find("kvgate:requests_total") != std::string::npos);
    CHECK(out.find("kvgate:affinity_spills_total") != std::string::npos);
    CHECK(out.find("kvgate:backend_failures_total") != std::string::npos);

    // TYPE headers present.
    CHECK(out.find("# TYPE kvgate:ttft_seconds histogram") != std::string::npos);
    CHECK(out.find("# TYPE kvgate:requests_running gauge") != std::string::npos);
    CHECK(out.find("# TYPE kvgate:requests_total counter") != std::string::npos);
    ...
    // Counter values.
    CHECK(out.find("kvgate:requests_total{code=\"200\"} 1") != std::string::npos);
    CHECK(out.find("kvgate:requests_running 3") != std::string::npos);
    CHECK(out.find("kvgate:requests_waiting 1") != std::string::npos);
}
...
TEST_CASE("metrics: histogram buckets are cumulative with +Inf, _sum, _count", "[metrics]") {
    ...
    CHECK(
        out.find("kvgate:ttft_seconds_bucket{route=\"chat_completions\",backend=\"b0\",model=\"m\","
                 "le=\"+Inf\"} 3") != std::string::npos);
    // _count is 3, _sum reflects the total.
    CHECK(
        out.find("kvgate:ttft_seconds_count{route=\"chat_completions\",backend=\"b0\",model=\"m\"} "
                 "3") != std::string::npos);
    CHECK(out.find("kvgate:ttft_seconds_sum") != std::string::npos);
...
TEST_CASE("metrics: distinct label sets produce distinct series", "[metrics]") {
    ...
    std::string out = reg.expose();
    CHECK(count_substr(out, "kvgate:ttft_seconds_count{") == 2); // one per backend
}
