// kvgate — OpenAI-compatible LLM gateway (single binary).
//
// Boost.Beast/Asio C++20-coroutine server that fronts configured backends,
// re-streaming SSE token-by-token with per-frame flush, injecting
// stream_options.include_usage upstream and forwarding the trailing usage
// chunk and [DONE]. SIGTERM triggers a graceful drain: new requests get 503
...
void print_usage() {
    std::cout << "kvgate " << kVersion << " — OpenAI-compatible LLM gateway\n"
              << "usage: kvgate --config <path.toml>\n"
              << "       kvgate --version\n"
              << "       kvgate --help\n";
}
} // namespace
...
int main(int argc, char** argv) {
    ...
    kvgate::config::Config cfg;
    try {
        cfg = kvgate::config::load_file(config_path);
    } catch (const kvgate::config::ConfigError& e) {
        std::cerr << "config error: " << e.what() << std::endl;
        return 2;
    }

    std::cout << "kvgate " << kVersion << " ready — " << config_path << " (" << cfg.backends.size()
              << " backend(s), policy=" << kvgate::config::to_string(cfg.routing.policy) << ")"
              << std::endl;

    try {
        asio::io_context ioc(1);
        kvgate::server::Gateway gateway(ioc, cfg);
        ...
        asio::co_spawn(ioc, kvgate::server::run_listener(endpoint, gateway),
        ...
        asio::co_spawn(ioc, kvgate::server::run_metrics_listener(metrics_endpoint, gateway),
        ...
        signals.async_wait([&](const boost::system::error_code&, int sig) {
            ...
            struct Drainer : std::enable_shared_from_this<Drainer> {
                asio::io_context& ioc;
                kvgate::server::Gateway& gw;
                asio::steady_timer timer;
                std::chrono::steady_clock::time_point deadline;
                Drainer(asio::io_context& i, kvgate::server::Gateway& g)
