...
// Metrics registry — the full kvgate Prometheus surface (locked names/buckets/
// labels, 03_PLAN.md + findings Q6). Handwritten text exposition, no dependency.
//
// Histograms (seconds, OTel GenAI-compatible buckets):
//   kvgate:ttft_seconds                  — TTFT buckets
//   kvgate:inter_token_latency_seconds   — TPOT buckets
//   kvgate:e2e_request_latency_seconds   — request-duration buckets
//   kvgate:request_queue_time_seconds    — admission queue wait
//   kvgate:gateway_overhead_seconds      — admission -> first upstream byte
//                                         forwarded (the self-overhead metric)
// Gauges:
//   kvgate:requests_running              — in-flight requests
//   kvgate:requests_waiting              — queued (admission) requests
// Counters:
//   kvgate:requests_total{code}          — completed requests by HTTP code
//   kvgate:affinity_spills_total         — prefix-affinity load-guard spills
//   kvgate:backend_failures_total        — upstream request failures
//
