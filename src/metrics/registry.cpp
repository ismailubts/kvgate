...
// DELIBERATE NAMING CONVENTION (do not "fix"): kvgate metric names are
// colon-prefixed (kvgate:ttft_seconds, ...) to mirror vLLM's / SGLang's de-facto
// serving-metric naming (vllm:..., findings Q6) so kvgate drops into existing
...
std::string Registry::expose() const {
    ...
    hist_block("kvgate:ttft_seconds", "Time to first token, seconds.", ttft_);
    hist_block("kvgate:inter_token_latency_seconds",
               "Inter-token (time-per-output-token) latency, seconds.", itl_);
    hist_block("kvgate:e2e_request_latency_seconds", "End-to-end request latency, seconds.", e2e_);
    hist_block("kvgate:request_queue_time_seconds", "Admission queue wait time, seconds.", queue_);
    hist_block("kvgate:gateway_overhead_seconds",
               "Gateway self-overhead: admission to first upstream byte forwarded, seconds.",
               overhead_);

    // Gauges.
    out += "# HELP kvgate:requests_running In-flight requests.\n";
    out += "# TYPE kvgate:requests_running gauge\n";
    out += "kvgate:requests_running " + std::to_string(requests_running_) + "\n";
    out += "# HELP kvgate:requests_waiting Requests waiting in the admission queue.\n";
    out += "# TYPE kvgate:requests_waiting gauge\n";
    out += "kvgate:requests_waiting " + std::to_string(requests_waiting_) + "\n";

    // Counters.
    out += "# HELP kvgate:requests_total Completed requests by HTTP status code.\n";
    out += "# TYPE kvgate:requests_total counter\n";
    for (const auto& [code, n] : requests_total_) {
        out += "kvgate:requests_total{code=\"" + esc(code) + "\"} " + std::to_string(n) + "\n";
    }
    out += "# HELP kvgate:affinity_spills_total Prefix-affinity load-guard spill cascades.\n";
    out += "# TYPE kvgate:affinity_spills_total counter\n";
    for (const auto& [labels, n] : affinity_spills_) {
        out += "kvgate:affinity_spills_total" + label_str(labels) + " " + std::to_string(n) + "\n";
    }
    out += "# HELP kvgate:backend_failures_total Upstream request failures.\n";
    out += "# TYPE kvgate:backend_failures_total counter\n";
    for (const auto& [labels, n] : backend_failures_) {
        out += "kvgate:backend_failures_total" + label_str(labels) + " " + std::to_string(n) + "\n";
    }
