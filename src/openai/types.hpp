...
// Only the subset kvgate v1 needs (chat completions, models). Unknown fields on
...
namespace kvgate::openai {

using json = nlohmann::json;

// A single chat message. `content` is kept as raw json because the OpenAI schema
// allows either a string or an array of content parts; kvgate does not need to
...
struct ChatCompletionRequest {
    ...
    // Parse from a request body. Throws kvgate::openai::RequestError (see
